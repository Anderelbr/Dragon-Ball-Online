...
{
	NTL_PRINT(PRINT_SYSTEM, "Packet Sended: [Opcode: [%i]] [Size: [%i]]", *(WORD*)pPacket->GetPacketData(), pPacket->GetPacketLen());
...
{
	NTL_PRINT(PRINT_SYSTEM, "Packet Sended: [Opcode: [%i]] [Size: [%i]]", *(WORD*)(pPacket->GetPacketData() + 2), *(WORD*)pPacket->GetPacketData());
...
//		Return	:
//-----------------------------------------------------------------------------------
int CNtlNetwork::Send(CNtlPacket * pPacket)
{
	NTL_PRINT(PRINT_SYSTEM, "Packet Sended To Everyone: [Opcode: [%i]] [Size: [%i]]", *(WORD*)(pPacket->GetPacketData() + 2), *(WORD*)pPacket->GetPacketData());
	...
	for (int i = 0; i < m_pSessionList->GetCurCount(); i++){

		CNtlSession *pSession = (CNtlSession*)m_pSessionList->Find((HSESSION)i);
		int rc = pSession->PushPacket(pPacket);
