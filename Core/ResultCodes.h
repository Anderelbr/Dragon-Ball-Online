...
enum ResultCodes {
	...
	CHARACTER_FAIL = 201,                 // 캐릭터 서버상의 전반적인 실패
	CHARACTER_AUTHKEY_FAIL = 202,         // 인증키 인증이 실패하였습니다.
	CHARACTER_TOO_LONG_NAME = 203,		// 캐릭터명의 길이가 초과되었습니다.
	CHARACTER_TOO_SHORT_NAME = 204,		// 캐릭터명의 길이가 너무 짧습니다.
	CHARACTER_SAMENAME_EXIST = 205,		// 이미 캐릭터명이 존재합니다.
	CHARACTER_COUNT_OVER = 206,           // 캐릭터 생성갯수가 초과되었습니다.
	CHARACTER_NAME_NOT_EXIST = 207,       // 캐릭터가 존재하지 않습니다.
	CHARACTER_CREATE_VALUE_FAIL = 208,	// 캐릭터 생성 값이 정확하지 않습니다. 
	CHARACTER_USER_EXIST = 209,           // 이미 접속중입니다.
	CHARACTER_USER_EXIST_IN_GAME_SERVER = 210,		// 게임 서버에 이미 플레이중인 캐릭터가 있습니다.
	CHARACTER_USER_ITEM_CREATE_FAIL = 211,// 캐릭터 아이템 생성이 실패하였습니다. 
	CHARACTER_USER_SKILL_CREATE_FAIL = 212,			// 캐릭터 스킬 생성이 실패하였습니다.
	CHARACTER_USER_QUICK_SLOT_CREATE_FAIL = 213,		// 캐릭터 퀵슬롯을 초기화하지 못했습니다.
	CHARACTER_DB_QUERY_FAIL = 214, // 캐릭터 서버에서 DB 쿼리가 실패하였습니다.
	CHARACTER_WRONG_SERVER_FARM_SELECTED = 215,		// 잘못된 서버를 선택하였습니다.
	CHARACTER_USER_BLOCK = 216,			// 캐릭터가 블록 되었습니다. 자세한 사항은 홈페이지를 문의하십시요.
	CHARACTER_BLOCK_STRING_INCLUDED = 217, // 부적합한 스트링이 포함되어 있습니다.
	CHARACTER_TEMPORARY_BLOCK = 218,		// 운영작업중이라 접속하실 수 없습니다. 잠시 후 접속을 시도해보십시요.
	CHARACTER_AUTHKEY_MISMATCH = 219,
	CHARACTER_AUTHKEY_CREATE_FAIL = 220,
	CHARACTER_AUTHKEY_NOT_FOUND = 221,
	CHARACTER_DELETE_GUILD_LEADER_FAIL = 222,			// 길드리더는 캐릭을 지울 수 없습니다.
	CHARACTER_DELETE_BUDOKAI_REGISTER_FAIL = 223,		// 천하제일무도회 신청중 일때는 캐릭터를 삭제할 수 없습니다.
	CHARACTER_SERVER_LOCKED = 224,					// 현재 서버가 잠긴 상태여서 접속할 수 없습니다.
	CHARACTER_DISCONNECTED_FORCEDLY = 225,			// 제재로 인해 강제로 접속이 종료되었습니다.
	CHARACTER_DISCONNECTED_TEMPORARILY = 226,			// 운영자에 의하여 일시적으로 로그아웃 처리가 되었습니다.
	CHARACTER_RACE_NOT_ALLOWED = 227,					// 허용되지 않은 종족 입니다.

	CHARACTER_COMMON_CANT_DO_THAT_FOR_SOME_REASON = 228,			// 시스템 문제로 작업을 처리할 수 없습니다.
	CHARACTER_USER_SHOULD_WAIT_FOR_CONNECT = 229,
	CHARACTER_SERVER_IS_UNDER_CONSTRUCTION = 230,					// 서버 점검 중 입니다.
